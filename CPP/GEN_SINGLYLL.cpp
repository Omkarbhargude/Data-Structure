...
/*template<class T>
BOOL SinglyLinkedList<T>::Reverse_GenList()
{
    struct GEN_SINGLYLIST<T> *pLast = NULL;
    struct GEN_SINGLYLIST<T> *current = NULL;
    struct GEN_SINGLYLIST<T> *Temp = NULL;
    
    current = pHead;

    if(pHead == NULL)
    {
        cout<<"[WARN] Cannot Reverse : List is empty\n";
        return FALSE;
    }
    ...
    while(current == pLast)
    {
        ...
        current->Data = pLast->Data;
        pLast->Data = Temp->Data;
...
int main()
{
    ...
    SinglyLinkedList<char> sobj;

    sobj.Insert_At_GenList_Last('A');
    sobj.Insert_At_GenList_Last('B');
    sobj.Insert_At_GenList_Last('C');
    sobj.Insert_At_GenList_Last('D');
    sobj.Insert_At_GenList_Last('A');

    sobj.Display_LIST();

    sobj.Reverse_GenList();
